...
const ThumballEdit = (props) => {
    ...
    const [encourage, setEncourage] = useState(1);
    ...
    useEffect(() => {
        getThumballById(props.match.params.id);
    ...
    const getThumballById = id => {

        props.firebase.thumball(id).get()
            .then(doc => {
                if (doc.exists) {
                    console.log("Thumball-----", doc.data());
                    var res = doc.data();
                    setTitle(res.title);
                    setThumballType(res.type);
                    setCategory(res.category);
                    setAge(res.age_group);
                    setTags(res.tags);
                    setDescription(res.description);
                    setAccess(res.access);
                    setPrimaryColor(res.color_1);
                    setSecondaryColor(res.color_2);
                    setSingleType(res.questiontype !== "");
                    setQuestionType(res.questiontype);
                    setEncourage(res.encourage);
                    setOften(res.often);
                    setPanelThatIs(res.panelthatis);
                    switch (res.type) {
                        case THUMBALLTYPE.CLASSIC12:
                            setClassic12(res.panel);
                            break;
                        case THUMBALLTYPE.CLASSIC32:
                            setClassic32(res.panel);
                            break;
                        case THUMBALLTYPE.CHALLENGE12:
                        case THUMBALLTYPE.CHALLENGE32:
                            setPanels(res.panel);
                            break;
                    }
                    setLoading(false);
    ...
    const updateThumball = () => {

        let date = Moment.tz(new Date(), "Europe/Paris").format();

        setLoading(true);

        var ref = props.firebase.thumball(props.match.params.id);
                                                                ...
                                                                <div className="row mt-4">
                                                                    <div className="col">
                                                                        ...
                                                                        <div className="d-flex align-items-center">
                                                                            <a href="#" className="clickhere">Click here for a sample.</a>
                                                                            <div className="ml-5">
                                                                                <div className="custom-radio form-check-inline m-0 mr-5">
                                                                                    <input className={encourage === 1 ? `custom-radio-input checked` : `custom-radio-input`} value="1" id="someRadio-322" name="radio-encourage" type="radio" onChange={e => setEncourage(e.target.value)} />
                                                                                ...
                                                                                <div className="custom-radio form-check-inline m-0">
                                                                                    <input className={encourage !== 1 ? `custom-radio-input checked` : `custom-radio-input`} value="0" id="someRadio-323" name="radio-encourage" type="radio" onChange={e => setEncourage(e.target.value)} />
                                                ...
                                                <div className="accordion-inner">
                                                    <div className="d-flex align-items-center justify-content-between">
                                                        ...
                                                        <div className="form-group my-4">
                                                            <button className="btn btn__submit gren" onClick={updateThumball}>Save</button>
                                                        </div>
                                                    </div>
...
export default compose(withFirebase, withAuthorization(condition))(ThumballEdit);